...
def _update_hash(v, update):
    ...
    if isinstance(v, (np.ndarray, np.generic) ):
        ...
        # Uses .item() to convert from numpy object to python
        
        if v.dtype is not np.dtype('O'): update( v.data.tobytes() )
        else: 
            for x in v: _update_hash(x.item(), update)
        return
    ...
    if isinstance(v, (list, set, tuple) ) :
        ...
        try:
            arr = np.array(v)
            if arr.dtype is not np.dtype('O'): update( arr.data.tobytes() )
            else: 
                for x in v: _update_hash(x, update)
            return
        ...
        if(len(members) > 0) :
            ...
            # if properties are not iterables use string repr
